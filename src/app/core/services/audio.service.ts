...
export class AudioService {
	...
	private statusStream: ReplaySubject<
		OnStatusCallbackData | OnStatusErrorCallbackData
	> = new ReplaySubject(1);
	private Log = console;
	...
	constructor(private platform: Platform, private zone: NgZone) {
		this.platform.ready().then(() => {
			if (this.platform.is("cordova")) {
				...
				if (!this.AudioPlayer) {
					this.Log.warn(
						"CordovaAudioPlayerService: Could not read `AudioPlayer` from `window.plugins`: ",
						(<any>window).plugins
					);
					throw new Error(
						"CordovaAudioPlayerService: Could not read `AudioPlayer` from `window.plugins`"
				...
				this.AudioPlayer.on(
					"status",
					(data: OnStatusCallbackData | OnStatusErrorCallbackData) => {
	...
	setOptions(options?: AudioPlayerOptions) {
		return this.wrapPromise("setOptions", (resolve, reject) => {
			this.AudioPlayer.setOptions(
				this.getSuccessCb(resolve),
				this.getErrorCb(reject),
				options
			);
	...
	setPlaylistItems = (items: AudioTrack[], options?: PlaylistItemOptions) => {
		return this.wrapPromise("setPlaylistItems", (resolve, reject) => {
			this.AudioPlayer.setPlaylistItems(
				this.getSuccessCb(resolve),
				this.getErrorCb(reject),
				items,
				options || {}
	...
	clearAllItems = () => {
		return this.wrapPromise("clearAllItems", (resolve, reject) => {
			this.AudioPlayer.clearAllItems(
				this.getSuccessCb(resolve),
				this.getErrorCb(reject)
			);
		}).catch(ex => console.warn(ex));
	...
	playTrackByIndex = (index: number) => {
		return this.wrapPromise("playTrackByIndex", (resolve, reject) => {
			this.AudioPlayer.playTrackByIndex(
				this.getSuccessCb(resolve),
				this.getErrorCb(reject),
				index
			);
	...
	playTrackById = (trackId: string) => {
		return this.wrapPromise("playTrackById", (resolve, reject) => {
			this.AudioPlayer.playTrackById(
				this.getSuccessCb(resolve),
				this.getErrorCb(reject),
				trackId
			);
	...
	seekToQueuePosition = (position: number) => {
		return this.wrapPromise("seekToQueuePosition", (resolve, reject) => {
			this.AudioPlayer.seekToQueuePosition(
				this.getSuccessCb(resolve),
				this.getErrorCb(reject),
				position
			);
	...
	getCurrentBuffer = () => {
		return this.wrapPromise("getCurrentBuffer", (resolve, reject) => {
			this.AudioPlayer.getCurrentBuffer(
				this.getSuccessCb(resolve),
				this.getErrorCb(reject)
			);
		}).catch(ex => console.warn(ex));
	...
	getQueuePosition = () => {
		return this.wrapPromise("getQueuePosition", (resolve, reject) => {
			this.AudioPlayer.getQueuePosition(
				this.getSuccessCb(resolve),
				this.getErrorCb(reject)
			);
		}).catch(ex => console.warn(ex));
	...
	private getSuccessCb = <T>(resolve: (value?: T | PromiseLike<T>) => void) => (
		data: any
	) => this.zone.run(() => resolve(data));
	private getErrorCb = (reject: (reason?: any) => void) => (data: any) =>
		this.zone.run(() => reject(data));
	...
	private async wrapPromise<T>(
		...
		try {
			...
			if (!this.AudioPlayer.isInitialized) {
				throw new Error(
					`cordova-plugin-playlist could not be initialized (calling [${name}])`
				);
			}
